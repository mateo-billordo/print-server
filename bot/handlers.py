...
def cmd_start(message):
    ...
    if not role:
        tgbot.reply_to(message, MSGS["pending_approval"])
        alert = MSGS["admin_access_request"].format(name=tg_name)
        try:
            tgbot.send_message(ADMIN_ID, alert, reply_markup=build_auth_menu(chat_id, tg_name))
        except Exception as e:
            log.error("Failed to notify admin: %s", e)
        return

    db_query("UPDATE users SET real_name = ? WHERE id = ?", (tg_name, chat_id), commit=True)
    tgbot.reply_to(message, MSGS["welcome"].format(name=tg_name), reply_markup=build_main_menu(chat_id))


@tgbot.message_handler(commands=['menu'])
def cmd_menu(message):
    chat_id = message.chat.id
    if not get_user_role(chat_id):
        return
    tgbot.send_message(chat_id, MSGS["menu_title"], reply_markup=build_main_menu(chat_id), parse_mode="Markdown")


# --- Text message handlers ---

@tgbot.message_handler(func=lambda m: m.text and "hola" in m.text.lower(), content_types=['text'])
def handle_greeting(message):
    ...
    tgbot.reply_to(message, MSGS["greeting"].format(name=display_name),
                 reply_markup=build_single_menu_button(), parse_mode="Markdown")


@tgbot.message_handler(func=lambda m: m.text and not m.text.startswith('/'), content_types=['text'])
def handle_text(message):
    ...
    if waiting == 1:
        ...
        if len(new_nickname) > 20:
            tgbot.reply_to(message, MSGS["nickname_too_long"])
            return
        set_nickname_state(chat_id, nickname=new_nickname, waiting=0)
        tgbot.reply_to(message, MSGS["nickname_saved"].format(nickname=new_nickname),
    ...
    if state == "waiting_add_email":
        ...
        if chat_id == ADMIN_ID and len(parts) == 2:
            ...
            try:
                ...
                tgbot.reply_to(message, MSGS["email_add_invalid_id"])
                return
        if add_user_email(target_id, addr):
            tgbot.reply_to(message, MSGS["email_added"].format(email=addr),
                         reply_markup=build_emails_sub_menu(chat_id), parse_mode="Markdown")
        else:
            tgbot.reply_to(message, MSGS["email_already_exists"].format(email=addr),
                         reply_markup=build_emails_sub_menu(chat_id), parse_mode="Markdown")
        return
    ...
    if state == "waiting_remove_email":
        ...
        if removed:
            tgbot.reply_to(message, MSGS["email_removed"].format(email=addr),
                         reply_markup=build_emails_sub_menu(chat_id), parse_mode="Markdown")
        else:
            tgbot.reply_to(message, MSGS["email_not_found"].format(email=addr),
                         reply_markup=build_emails_sub_menu(chat_id), parse_mode="Markdown")
        return
    ...
    if state == "waiting_ecfg_address":
        ...
        tgbot.reply_to(message, MSGS["email_config_updated"].format(field="dirección", value=val),
    ...
    if state == "waiting_ecfg_password":
        ...
        email_wake_event.set()
        tgbot.reply_to(message, MSGS["email_config_updated"].format(field="contraseña", value="••••••••"),
                     reply_markup=build_email_config_sub_menu(), parse_mode="Markdown")
        return
    ...
    if state == "waiting_ecfg_timer":
        try:
            ...
            tgbot.reply_to(message, "⚠️ Ingresá un número válido.")
            return
        set_email_config("timer_minutes", val)
        email_wake_event.set()
        tgbot.reply_to(message, MSGS["email_config_updated"].format(
            field="timer", value=f"{val} min" if val > 0 else "desactivado"),
                     reply_markup=build_email_config_sub_menu(), parse_mode="Markdown")
        return

    tgbot.reply_to(message, MSGS["unknown_command"],
                 reply_markup=build_single_menu_button(), parse_mode="Markdown")


# --- File/photo handler ---

@tgbot.message_handler(content_types=['document', 'photo'])
def handle_file(message):
    ...
    if message.content_type == 'document':
        file_info = tgbot.get_file(message.document.file_id)
        file_name = message.document.file_name
    else:
        file_info = tgbot.get_file(message.photo[-1].file_id)
        file_name = f"photo_{message.message_id}.jpg"

    downloaded = tgbot.download_file(file_info.file_path)
...
def handle_callback(call):
    ...
    if call.data.startswith("auth_"):
        ...
        tgbot.send_message(target_id, MSGS["user_auth_granted"].format(admin=call.from_user.first_name),
    ...
    if call.data == "nickname_change":
        ...
        tgbot.edit_message_text(MSGS["nickname_prompt"], chat_id, msg_id)
        tgbot.answer_callback_query(call.id)
        return

    if call.data == "nickname_delete":
        set_nickname_state(chat_id, nickname="", waiting=0)
        tgbot.edit_message_text(MSGS["nickname_deleted"], chat_id, msg_id, reply_markup=build_main_menu(chat_id))
        tgbot.answer_callback_query(call.id)
        return
    ...
    if call.data == "menu_back":
        tgbot.answer_callback_query(call.id)
        tgbot.edit_message_text(MSGS["menu_title"], chat_id, msg_id,
                             reply_markup=build_main_menu(chat_id), parse_mode="Markdown")
        return
    ...
    if call.data == "menu_apodo":
        ...
        tgbot.edit_message_text(text, chat_id, msg_id, reply_markup=markup, parse_mode="Markdown")
        return
    ...
    if call.data == "menu_ayuda":
        ...
        tgbot.edit_message_text(help_text, chat_id, msg_id, reply_markup=markup, parse_mode="Markdown")
        return
    ...
    if call.data == "menu_users_sub":
        if chat_id != ADMIN_ID:
            return
        tgbot.answer_callback_query(call.id)
        tgbot.edit_message_text(MSGS["sub_users_title"], chat_id, msg_id,
    ...
    if call.data == "usub_list":
        ...
        tgbot.edit_message_text(response, chat_id, msg_id,
                             reply_markup=build_users_sub_menu(), parse_mode="Markdown")
        return
    ...
    if call.data == "usub_remove":
        if chat_id != ADMIN_ID:
            return
        tgbot.answer_callback_query(call.id)
        tgbot.edit_message_text(MSGS["sub_users_select_remove"], chat_id, msg_id,
    ...
    if call.data == "usub_role":
        if chat_id != ADMIN_ID:
            return
        tgbot.answer_callback_query(call.id)
        tgbot.edit_message_text(MSGS["sub_users_select_role"], chat_id, msg_id,
    ...
    if call.data.startswith("rmuser_"):
        ...
        tgbot.answer_callback_query(call.id)
        tgbot.edit_message_text(MSGS["user_removed"].format(name=name), chat_id, msg_id,
    ...
    if call.data == "menu_emails_sub":
        tgbot.answer_callback_query(call.id)
        tgbot.edit_message_text(MSGS["sub_emails_title"], chat_id, msg_id,
                             reply_markup=build_emails_sub_menu(chat_id), parse_mode="Markdown")
        return
    ...
    if call.data == "esub_list":
        ...
        tgbot.edit_message_text(text, chat_id, msg_id,
                             reply_markup=build_emails_sub_menu(chat_id), parse_mode="Markdown")
        return
    ...
    if call.data == "esub_add":
        ...
        tgbot.edit_message_text(MSGS["email_prompt_add"], chat_id, msg_id)
        return

    if call.data == "esub_remove":
        tgbot.answer_callback_query(call.id)
        user_state[chat_id] = "waiting_remove_email"
        tgbot.edit_message_text(MSGS["email_prompt_remove"], chat_id, msg_id)
        return
    ...
    if call.data == "esub_config":
        if chat_id != ADMIN_ID:
            return
        tgbot.answer_callback_query(call.id)
        tgbot.edit_message_text(MSGS["sub_ecfg_title"], chat_id, msg_id,
                             reply_markup=build_email_config_sub_menu(), parse_mode="Markdown")
        return
    ...
    if call.data == "ecfg_address":
        ...
        tgbot.edit_message_text(MSGS["ecfg_prompt_address"], chat_id, msg_id)
        return
    ...
    if call.data == "ecfg_password":
        ...
        tgbot.edit_message_text(MSGS["ecfg_prompt_password"], chat_id, msg_id)
        return
    ...
    if call.data == "ecfg_timer":
        ...
        tgbot.edit_message_text(MSGS["ecfg_prompt_timer"], chat_id, msg_id)
        return
    ...
    if call.data == "menu_monitor_sub":
        if chat_id != ADMIN_ID:
            return
        tgbot.answer_callback_query(call.id)
        tgbot.edit_message_text(MSGS["sub_monitor_title"], chat_id, msg_id,
    ...
    if call.data == "tsub_reset":
        ...
        tgbot.answer_callback_query(call.id)
        reset_ink_counters()
        tgbot.edit_message_text(MSGS["ink_reset_success"], chat_id, msg_id,
    ...
    if call.data == "tsub_testpage":
        ...
        if result.returncode == 0:
            tgbot.edit_message_text(MSGS["testpage_sent"], chat_id, msg_id,
                                 reply_markup=build_monitor_back_button())
        else:
            log.error("Test page failed: %s", result.stderr)
            tgbot.edit_message_text(MSGS["testpage_error"], chat_id, msg_id,
                                 reply_markup=build_monitor_back_button())
        return
    ...
    if call.data == "tsub_monitor":
        ...
        tgbot.edit_message_text(status_text, chat_id, msg_id,
    ...
    if call.data == "tsub_reactivar":
        ...
        if result == "ok":
            tgbot.edit_message_text(MSGS["printer_reactivated"], chat_id, msg_id,
                                 reply_markup=build_monitor_back_button())
        else:
            tgbot.edit_message_text(
    ...
    with jobs_lock:
        if chat_id not in user_jobs:
            tgbot.answer_callback_query(call.id, MSGS["job_expired"])
            return
    ...
    if call.data == "copies_plus":
        with jobs_lock:
            job["copies"] += 1
        tgbot.edit_message_reply_markup(chat_id, msg_id, reply_markup=build_print_menu(chat_id))
        tgbot.answer_callback_query(call.id, str(job["copies"]))
        ...
        try:
            ...
            pass
        tgbot.edit_message_text(MSGS["cancelled"], chat_id, msg_id)
        tgbot.answer_callback_query(call.id)

    elif call.data == "print_confirm":
        with jobs_lock:
            user_jobs.pop(chat_id, None)
        tgbot.edit_message_text(MSGS["sending_to_queue"], chat_id, msg_id)
        tgbot.answer_callback_query(call.id)
